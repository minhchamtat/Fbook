    ...
    <div class="product no-slide image">
        <div class="product-img image">
            <a href="{{ route('user', $bestSharing->user->id) }}" title="{{ $bestSharing->user->name }}">
                <img src="{{ ($bestSharing->user->avatar) ? $bestSharing->user->avatar : asset(config('view.image_paths.user') . '1.png') }}" alt="book" class="primary-image" onerror="this.onerror=null;this.src={{ config('view.links.avatar') }};" />
